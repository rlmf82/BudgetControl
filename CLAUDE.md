...
- **Persistence**: MySQL is the default datasource (`spring.datasource.*` in `application.properties`, pointed at `jdbc:mysql://localhost:3306/budgetcontrol`, credentials via `DB_USERNAME`/`DB_PASSWORD` env vars defaulting to `root`/empty) — you must create the `budgetcontrol` schema yourself and have MySQL reachable; Liquibase creates the tables on startup. H2 is only used for tests (`src/test/resources/application.properties`, in-memory). `spring.jpa.hibernate.ddl-auto=validate` — schema changes always go through a new Liquibase changeset under `src/main/resources/db/changelog/changes/`, never by letting Hibernate generate DDL.
